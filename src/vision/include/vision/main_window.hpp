...
private slots:
    ...
    void onMoveRedLineCheckChanged(bool checked);
    ...
    void onStopAndTrafficClicked();
    void onBlockAndGateClicked();
...
private:
    ...
    // ===== 차선 검출 및 발행 =====
    void findLaneAndPublish(const cv::Mat &mask);
    void publishLaneInfo(float offset_m, float angle, bool left_found, bool right_found);

    // ===== 퍼블리셔 =====
    rclcpp::Publisher<vision_msgs::msg::LaneInfo>::SharedPtr lane_pub_;
    ...
    cv::Scalar lower_red_line, upper_red_line;
